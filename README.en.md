...
| `ixfdoc outline <file> --json` | Build heading-aware dynamic reading metadata |
| `ixfdoc chunk <file> --index N` | Read one dynamic chunk without creating split files |
| `ixfdoc cleanup <out-dir>` | Remove generated artifacts after text and image analysis |
...
For agent reads with images, use `--download-images`, read every `outline` /
`chunk`, inspect every local path in `imagePaths`, and then run `ixfdoc
cleanup <out-dir>`. The complete Markdown is still written as one file.
...
The smoke scripts require exactly one wheel under `dist/` and verify its package version, CLI entry point, and bundled Codex skill in a temporary virtual environment and HOME. They do not use a globally installed `ixfdoc`.
