...
Use `--cleanup` when the generated Markdown/TSV artifacts are only needed during the current run:

```bash
out="$(mktemp -d /tmp/ixfdoc.XXXXXX)"
ixfdoc read <url-or-file>... --out-dir "$out" --expand-sheets --print-manifest --cleanup
```
